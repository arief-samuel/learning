...
var accountNames = new[] {"arief","juita","ephra"};
var random = new Random();

while (!Console.KeyAvailable)
{
    var atm = client.GetGrain<IAtmGrain>(0);
    //Choose some random accounts to exchane
    var fromId = random.Next(accountNames.Length); 
    var toId = random.Next(accountNames.Length);
    while (toId == fromId)
    {
        //Avoid transfering to/from same account, since it would be meaningless
    ...
    {
        Console.WriteLine(
            $"Error transfered 100 credits from {fromName} to " +
            $"{fromName}.\n{toName} exception: {exception.Message}\n{toName}");
